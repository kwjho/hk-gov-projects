#!/usr/bin/env python3
"""
Fetches HYD Road Projects from CSDI WFS + HYD project detail pages (EN + TC).
Geometry is MultiLineString; centroid is used for map marker placement.
Outputs data/hyd_road.json used by the webapp.
"""
...
DATASET_ID = "hyd_road"
DATASET_NAME = {"en": "Road Projects", "tc": "道路工程項目"}
DEPARTMENT = "HYD"
DEPARTMENT_FULL = {"en": "Highways Department", "tc": "路政署"}

WFS_URL = (
    "https://portal.csdi.gov.hk/server/services/common/"
    "hyd_rcd_1728895888022_19427/MapServer/WFSServer"
    "?service=wfs&request=GetFeature&typenames=Road_Project"
...
def multiline_centroid(geometry):
    """Average all vertices of a MultiLineString (or LineString) for map placement."""
    if not geometry:
        return None
...
def code_from_url(url):
    m = re.search(r"/road_projects/([^/]+)/", url)
    return m.group(1) if m else ""


def main():
    print("Fetching HYD Road Projects WFS...")
    ...
    for i, feat in enumerate(features, 1):
        ...
        centroid = multiline_centroid(geom)

        print(f"  [{i:03d}/{len(features)}] {code} S{status} | {name_en[:55]}...")
        ...
        projects.append({
            ...
            "name":       name_en,
            "type":       None,
            "type_label": {"en": "Road Project", "tc": "道路工程"},
            "status":     status,
